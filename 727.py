...
if __name__ == '__main__':
    ...
    aas=[]
    namelist=[]
    for imgidx in range(1,60):#[55]:#(list(range(11,51))+list(range(56,60))):#,1,2,3,4,6,7,8,9,10,51,52,53,54,55]:
        namelist.append('rgtreg'+str(imgidx))
        namelist.append('rinreg' + str(imgidx))
    ...
    for name in namelist:
        rgt = np.load('F:\SERT-master\models\competing_methods\\'+name+'.npy') /4096
        ...
        for j in range(len(peaks)-1):
            ...
            model_ransac = linear_model.RANSACRegressor(min_samples=min(10,len(stds)))
            model_ransac.fit(means.reshape(-1, 1), stds, )
        ...
        #     try:
        #          model_ransac = linear_model.RANSACRegressor(min_samples=10)
        #          model_ransac.fit(means.reshape(-1, 1), stds,)
        #         k = model_ransac.estimator_.coef_[0]
        #         b = model_ransac.estimator_.intercept_
        #         if k>0:
